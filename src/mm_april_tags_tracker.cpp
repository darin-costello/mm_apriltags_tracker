...
pair<double, double> pairAdd(pair<double, double> one, pair<double, double>two){
...
void MMAprilTagsTracker::calibrate(vector<AprilTags::TagDetection> tags){
  ...
  for(auto t:tags){
    if(groundLocs.end() != groundLocs.find(t.id)){
      for(int i = 0; i < 4; i++){
        cout << "Id: " << i << " (" << t.p[i].first << " " << t.p[i].second <<")\n";
        srcPoints.push_back(Point2f(t.p[i].first, t.p[i].second));
        pair<double,double> dst = pairAdd(groundLocs[t.id], offSets[i]);
        dstPoints.push_back(Point2f(dst.first, dst.second));
    }
      foundNumber += 4;
    }
